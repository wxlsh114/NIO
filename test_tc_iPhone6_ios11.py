...
from common_ios11 import fun_myfansui_check
from common_ios11 import fun_mywatchui_check
from common_ios11 import fun_mypublishui_check
from common_ios11 import bp_is_loggedin
from common_ios11 import fun_getinfo
from common_ios11 import bp_normalloginmp
from common_ios11 import bp_is_loginshow
from common_ios11 import fun_getloginmenu
from common_ios11 import bp_is_plusexist
from common_ios11 import bp_is_publishnowexist
from common_ios11 import bp_is_openmultichatexist
from common_ios11 import fun_findui_check
from common_ios11 import fun_personalinfoui_check
from common_ios11 import fun_scoredetailui_check
from common_ios11 import fun_giftui_check
from common_ios11 import fun_articleui_check
from common_ios11 import fun_cartui_check
from common_ios11 import fun_pgcui_check
#from common_ios11 import fun_articledetailui_check
#from common_ios11 import fun_giftorderui_check
#from common_ios11 import fun_giftorderdetailui_check
from common_ios11 import fun_getjingxiloginmenu
...
#*******************************************************
    def test_jingxi_products_tc013(self):
        ...
        sleep(8)
        for i in range(3):
            driver.execute_script('mobile: dragFromToForDuration',{'fromX':50,'fromY':550,'toX':50,'toY':70,'duration':1.0})
            sleep(2)
...
#*********************************************************************************************
    def test_jingxi_add2basket_tc022(self):
        ...
        driver.execute_script('mobile: dragFromToForDuration',{'fromX':50,'fromY':550,'toX':50,'toY':150,'duration':1.0})
        sleep(2)
        u=driver.find_elements_by_xpath('//XCUIElementTypeOther[@name="轻奢长柄伞 200"]')
        if len(u) != 0:
            print('需要兑换的商品存在，检查通过')
            sleep(2)
            driver.find_element_by_xpath('//XCUIElementTypeOther[@name="轻奢长柄伞 200"]').click()
            sleep(9)
        ...
        if len(chk0) != 0:
            ...
            if chk == True:
                ...
                try:
                    ...
                    driver.find_element_by_accessibility_id('确定').click()
                    sleep(3)
                    chk=driver.find_elements_by_xpath('//XCUIElementTypeOther[contains(@name,"订单已提交")]')
                    sleep(1)
                    if len(chk) == 0:
                        ...
                        sleep(6)
                        #检查商品状态
                        chk1=driver.find_elements_by_xpath('//XCUIElementTypeOther[contains(@name,"轻奢长柄伞")]')
...
#*******************************************************
    def test_faxian_expershowpic_tc032(self):
        ...
        show=driver.find_elements_by_xpath('//*[@name="晒图"]')
        sleep(2)
        if len(show) != 0:
            print('晒图按钮存在,检查通过')
            sleep(1)
            ...
            sleep(2)
        #driver.execute_script("mobile: tap", {"touchCount":"1", "x":32, "y":45})
            ...
            if len(sh) != 0:
                ...
                if len(wh) != 0:
                    ...
                    if len(words) != 0:
                        ...
                        sleep(1)
                        now0=time.strftime('%Y-%m-%d %H_%M_%S')
                        words[0].send_keys('测试PGC微信好友:'+now0)
                        sleep(1)
                    #发送
                    driver.find_element_by_accessibility_id('发送').click()
                    sleep(2)
...
#*******************************************************
    def test_faxian_experjoin_tc038(self):
        ...
        sleep(2)
        driver.find_element_by_accessibility_id('更多同城活动').click()
        sleep(2)
        for i in range(8):
            driver.execute_script('mobile: dragFromToForDuration',{'fromX':50,'fromY':550,'toX':50,'toY':50,'duration':1.0})
            sleep(2)
...
#************************************************************************************************************
    def test_jingxi_cartcheck_tc043(self):
        ...
        driver.execute_script('mobile: dragFromToForDuration',{'fromX':50,'fromY':550,'toX':50,'toY':150,'duration':1.0})
        sleep(2)
        u1=driver.find_elements_by_xpath('//XCUIElementTypeOther[@name="轻奢长柄伞 200"]')
        if len(u1) != 0:
            ...
            if len(add2b) != 0:
                ...
                if len(ch1) != 0:
                    ...
                    driver.execute_script('mobile: tap', {'touchCount':'1', 'x':34, 'y':58})
                    sleep(2)
                    u2=driver.find_elements_by_xpath('//XCUIElementTypeOther[@name="蔚来创意笔盒套装 250"]')
                    if len(u2) != 0:
                        print('需要兑换的商品2存在，检查通过')
                        sleep(2)
                        driver.find_element_by_xpath('//XCUIElementTypeOther[@name="蔚来创意笔盒套装 250"]').click()
                        sleep(10)
                        page=driver.page_source
                        sleep(2)
...
#*******************************************************
    def test_wode_activityshow_tc046(self):
        ...
        if len(u) != 0:
            ...
            sleep(2)
            #check numbers of pictures and published text here
            title=driver.find_elements_by_xpath('//*[contains(@name,t0)]')
            sleep(2)
            if len(title) != 0 :
                print('晒图的文字检查通过')
                sleep(1)
                ...
                driver.get_screenshot_as_file(sf1)
            sleep(2)
            number=driver.find_elements_by_xpath('//*[@name="9"]')
            if len(number) != 0:
                print('晒图的上传9张图片检查通过')
                sleep(1)
            else:
