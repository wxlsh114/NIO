...
def bp_normalloginmp(self):
    ...
    #f=fun_getinfo(self)
    #sleep(2)
    driver.execute_script("mobile: dragFromToForDuration",{"fromX":50,"fromY":250,"toX":50,"toY":500,"duration":1.0})
    sleep(1)
    ...
    driver.execute_script("mobile: tap", {"touchCount":"1", "x":160, "y":354})
    sleep(5)
    #driver.execute_script("mobile: dragFromToForDuration",{"fromX":50,"fromY":250,"toX":50,"toY":550,"duration":1.0})
    #driver.execute_script("mobile: scroll", {"direction": "up"})
    #sleep(1)
    name=driver.find_elements_by_xpath('//XCUIElementTypeStaticText[contains(@name,"车主多积分")]')
    if len(name) != 0:
        print('登录成功')
    else:
        print('登录失败')
