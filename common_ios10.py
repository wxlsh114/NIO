...
def fun_getinfo(self):
    ...
    sleep(1)
    #iPhone5S/iOS10.2.1/98762648229
    ...
    mobile_no0=int(st1.cell(5,0).value)
    print('手机号：'+str(mobile_no0))
    code0=st1.cell(5,1).value
    print('验证码：'+code0)
    sleep(1)
...
def bp_is_publishnowexist(self):
    ...
    #driver.find_element_by_accessibility_id('发现').click()
    #sleep(2)
    publish=driver.find_elements_by_accessibility_id('发布此刻')
    if len(publish) == 0:
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf='../../test_report/ios/'+now+'_errorPlus_R.png'
        driver.get_screenshot_as_file(sf)
...
def bp_is_openmultichatexist(self):
    ...
    #driver.find_element_by_accessibility_id('发现').click()
    #sleep(2)
    publish=driver.find_elements_by_accessibility_id('发起群聊')
    if len(publish) == 0:
        now=time.strftime('%Y-%m-%d %H_%M_%S')
        sf='../../test_report/ios/'+now+'_errorPlus_R.png'
        driver.get_screenshot_as_file(sf)
...
def bp_is_loggedin(self):
    ...
    if len(login) == 0:
        ...
        driver.execute_script("mobile: dragFromToForDuration",{"fromX":150,"fromY":550,"toX":150,"toY":250,"duration":1.0})
        sleep(2)
        #设置
...
def fun_getmyfans(self):
    ...
    st1=workbook.sheet_by_name('SheetiOS10')
    listM=[]
    listF=[]
    for i in range(1,6):
        xpath_checked=str(st1.cell(i,0).value)
...
def bp_normalloginmp(self):
    ...
    driver.execute_script("mobile: dragFromToForDuration",{"fromX":150,"fromY":250,"toX":150,"toY":550,"duration":1.0})
    sleep(2)
    name=driver.find_elements_by_xpath('//XCUIElementTypeStaticText[contains(@name,"Sam8")]')
    if len(name) != 0:
        print('登录成功！')
    else:
        print('登录失败！')
