...
from common_iPhone7_ios12_stg import fun_myfansui_check
from common_iPhone7_ios12_stg import fun_mywatchui_check
from common_iPhone7_ios12_stg import fun_mypublishui_check
from common_iPhone7_ios12_stg import bp_is_loggedin
from common_iPhone7_ios12_stg import fun_getinfo
from common_iPhone7_ios12_stg import bp_normalloginmp
from common_iPhone7_ios12_stg import bp_is_loginshow
from common_iPhone7_ios12_stg import fun_getloginmenu
from common_iPhone7_ios12_stg import bp_is_plusexist
from common_iPhone7_ios12_stg import bp_is_publishnowexist
from common_iPhone7_ios12_stg import bp_is_openmultichatexist
from common_iPhone7_ios12_stg import fun_findui_check
from common_iPhone7_ios12_stg import fun_personalinfoui_check
from common_iPhone7_ios12_stg import fun_scoredetailui_check
from common_iPhone7_ios12_stg import fun_giftui_check
from common_iPhone7_ios12_stg import fun_articleui_check
from common_iPhone7_ios12_stg import fun_cartui_check
from common_iPhone7_ios12_stg import fun_pgcui_check
#from common_iPhone7_ios12_stg import fun_articledetailui_check
#from common_iPhone7_ios12_stg import fun_giftorderui_check
#from common_iPhone7_ios12_stg import fun_giftorderdetailui_check
from common_iPhone7_ios12_stg import fun_getjingxiloginmenu
from common_iPhone7_ios12_stg import bp_deleteaddress
from common_iPhone7_ios12_stg import fun_pinch
from common_iPhone7_ios12_stg import fun_zoom
from common_iPhone7_ios12_stg import bp_is_initpicexist
from common_iPhone7_ios12_stg import fun_lightedui_check
from common_iPhone7_ios12_stg import fun_notlightedui_check
from common_iPhone7_ios12_stg import fun_mostsiteui_check
from common_iPhone7_ios12_stg import fun_activityui_check
from common_iPhone7_ios12_stg import fun_livecastui_check
from common_iPhone7_ios12_stg import bp_normalloginmp_carowner
from common_iPhone7_ios12_stg import bp_normalloginmp_notenoughscore
from common_iPhone7_ios12_stg import bp_normalloginmp_zeroscore
...
#*******************************************************
    def test_faxian_livecast_tc017(self):
        ...
        for i in range(4):
            self.driver.execute_script("mobile: scroll", {"direction": "down"})
            sleep(1)
        driver.execute_script("mobile: dragFromToForDuration",{"fromX":5,"fromY":100,"toX":5,"toY":400,"duration":1.0})
        sleep(2)
...
#*******************************************************
    def test_faxian_article_tc019(self):
        ...
        for i in range(3):
            driver.execute_script("mobile: scroll", {"direction": "down"})
            sleep(1)
        #driver.execute_script('mobile: dragFromToForDuration',{'fromX':5,'fromY':250,'toX':5,'toY':260,'duration':1.0})
        sleep(2)
        #检查文章的各个元素是否存在
        ...
        if c1 == True:
            ...
            sleep(2)
            #点击评论
            driver.find_element_by_xpath('//XCUIElementTypeTable/XCUIElementTypeCell[5]/XCUIElementTypeButton[3]').click()
            sleep(3)
...
#*******************************************************
    def test_faxian_infopgcshare_tc037(self):
        ...
        for i in range(15):
            driver.execute_script("mobile: scroll", {"direction": "down"})
            sleep(1)
        driver.execute_script('mobile: dragFromToForDuration',{'fromX':5,'fromY':440,'toX':5,'toY':240,'duration':1.0})
        sleep(2)
        #检查发现页面资讯tab下的pgc的的各个元素是否存在
...
#*******************************************************
    def test_faxian_experjoin_tc038(self):
        ...
        if len(joins) != 0:
            ...
            driver.find_element_by_accessibility_id('完成').click()
            sleep(1)
            driver.execute_script('mobile: tap', {'touchCount':'1', 'x':343, 'y':232})
...
#*******************************************************
    def test_aiche_es8content_tc052(self):
        ...
        if len(cmap) != 0:
            print('详细配置表按钮找到')
            sleep(2)
            driver.find_element_by_xpath('//XCUIElementTypeTable/XCUIElementTypeCell[10]/XCUIElementTypeButton[6]').click()
...
#*********************************************************************************************
    def test_faxian_lightenchina4carowner_tc061(self):
        ...
        if len(ch) != 0:
            ...
            sleep(2)
            #探索附近
            driver.execute_script("mobile: dragFromToForDuration",{"fromX":5,"fromY":550,"toX":5,"toY":100,"duration":1.0})
            sleep(2)
            #3个检查点
            ...
            chk3=driver.find_elements_by_xpath('//XCUIElementTypeOther[@name="点亮中国"]/XCUIElementTypeLink[6]/XCUIElementTypeLink[3]')
            sleep(1)
            #print(len(chk1))
            #print(len(chk2))
            #print(len(chk3))
            ...
            sleep(2)
            #导航按钮
            driver.find_element_by_xpath('//XCUIElementTypeOther[@name="点亮中国"]/XCUIElementTypeLink[6]/XCUIElementTypeLink[3]').click()
            sleep(1)
            #发送
            driver.find_element_by_accessibility_id('发送').click()
            #sleep(0.2)
...
#*******************************************************
    def test_faxian_infospecialshare_tc070(self):
        ...
        for i in range(4):
            driver.execute_script("mobile: scroll", {"direction": "down"})
            sleep(1)
        driver.execute_script('mobile: dragFromToForDuration',{'fromX':50,'fromY':100,'toX':50,'toY':450,'duration':1.0})
        sleep(2)
...
#*********************************************************************************************
    def test_jingxi_notenoughscore_tc074(self):
        ...
        #driver.execute_script("mobile: dragFromToForDuration",{"fromX":50,"fromY":300,"toX":50,"toY":450,"duration":1.0})
        sleep(3)
        u1=driver.find_elements_by_xpath('//XCUIElementTypeOther[@name="自动化专用纯积分 自动化专用别动 10000"]')
        #ch0=driver.find_element_by_xpath('//XCUIElementTypeOther[@name="自动化专用纯积分 自动化专用别动 10000"]').get_attribute('visible')
        sleep(1)
        if len(u1) != 0:
            print('纯积分商品找到')
            sleep(2)
            driver.find_element_by_xpath('//XCUIElementTypeOther[@name="自动化专用纯积分 自动化专用别动 10000"]').click()
            sleep(9)
            #加入购物车
...
#*******************************************************
    def test_aiche_rechargemapownercommentslike_tc092(self):
        ...
        if len(cmap) != 0:
            ...
            driver.execute_script("mobile: tap", {"touchCount":"1", "x":268, "y":239})
            sleep(5)
            driver.execute_script('mobile: dragFromToForDuration',{'fromX':150,'fromY':500,'toX':150,'toY':50,'duration':1.0})
            sleep(2)
            #点赞
    ...
    def test_aiche_rechargemaprechargepole_tc095(self):
        ...
        if len(cmap) != 0:
            ...
            if len(pi) == 0:
                ...
                sleep(5)
                px=driver.find_elements_by_xpath('//XCUIElementTypeStaticText[contains(@name,"安亭汽车创新港换电站")]')
                if len(px) == 0:
    ...
    def test_aiche_rechargemapsearchroute_tc096(self):
        ...
        if len(cmap) != 0:
            ...
            if len(ch1) == 0:
                ...
                sleep(2)
                #过滤
                driver.execute_script("mobile: tap", {"touchCount":"1", "x":337, "y":214})
    ...
    def test_aiche_rechargemaprouteplan_tc097(self):
        ...
        if len(cmap) != 0:
            ...
            if len(ch1) == 0:
                ...
                sleep(1)
                #下拉
                driver.execute_script('mobile: dragFromToForDuration',{'fromX':50,'fromY':200,'toX':50,'toY':500,'duration':1.0})
                sleep(2)
                #过滤
                driver.execute_script("mobile: tap", {"touchCount":"1", "x":337, "y":174})
                ...
                #zoom
                driver.execute_script('mobile:pinch',{'scale':2.5,'velocity':1.5,'element':p})
                sleep(2)
                #换电站
                driver.execute_script("mobile: tap", {"touchCount":"1", "x":182, "y":362})
                sleep(2)
                #上拉
                driver.execute_script('mobile: dragFromToForDuration',{'fromX':50,'fromY':550,'toX':50,'toY':100,'duration':1.0})
                sleep(3)
